...
export class prescriptionController implements Controller {
    ...
    private initializeRoutes() {
        this.router.delete('/prescription/:id', this.GpDelete);
this.router.get('/prescription/get/search', this.GpSearch);
this.router.put('/prescription/get/update', this.GpSearchForUpdate);
this.router.put('/prescription', this.GpUpdate);
this.router.get('/prescription/:id', this.GpGetNounById);
this.router.get('/prescription', this.GpGetAllValues);
this.router.post('/prescription', this.GpCreate);
    }

public GpDelete(req: Request, res: Response) {
            new CustomLogger().showLogger('info', 'Enter into prescriptionController.ts: GpDelete');
        new ApiAdapter().delete(Constant.PRESCRIPTIONMANAGERURL + `${req.url}` )
...
public GpSearch(req: Request, res: Response) {
        ...
        new ApiAdapter().get(Constant.PRESCRIPTIONMANAGERURL + `${req.url}` )
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpSearchForUpdate(req: Request, res: Response) {
        ...
        new ApiAdapter().put(Constant.PRESCRIPTIONMANAGERURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
...
public GpCreate(req: Request, res: Response) {
        ...
        new ApiAdapter().post(Constant.PRESCRIPTIONMANAGERURL + `${req.url}` , req.body)
        .then((res: any) => res.response.json()).then(result => {
              req.baseUrl === '/mobile' ? res.send(result) :
              req.baseUrl === '/web' ? res.send(result) : res.send(null)
